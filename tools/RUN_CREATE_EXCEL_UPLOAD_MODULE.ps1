# File: tools\RUN_CREATE_EXCEL_UPLOAD_MODULE.ps1
# Purpose: Create Admin Excel Upload module (Master/Itemmaster/Itemevent) with fast chunk upload + progress UI + upload timestamps
# Run: pwsh -ExecutionPolicy Bypass -File .\tools\RUN_CREATE_EXCEL_UPLOAD_MODULE.ps1

Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

trap {
  try { Write-Host ("[FATAL] " + ($_ | Out-String)) -ForegroundColor Red } catch {}
  exit 1
}
...
function Backup-Dir([string]$SrcPath, [string]$BackupRoot) {
  if (-not (Test-Path -LiteralPath $SrcPath)) { return }
  New-Dir $BackupRoot
  $name = Split-Path -Leaf $SrcPath
  $dest = Join-Path $BackupRoot $name
  Copy-Item -LiteralPath $SrcPath -Destination $dest -Recurse -Force
}

function Save-LastBackupDir([string]$RepoRoot, [string]$BackupDir) {
  $tools = Join-Path $RepoRoot "tools"
  New-Dir $tools
  $file = Join-Path $tools "LAST_BACKUP_DIR.txt"
...
function Run-Npm([string]$WorkingDir, [string[]]$Args) {
  ...
  if ($p.ExitCode -ne 0) { throw "npm failed exit=$($p.ExitCode): npm $argLine" }
...
$script:LogFile = Join-Path $logsDir ("run_create_excel_upload_module_{0}.log" -f (Now-Stamp))
Write-Log "RepoRoot: $repoRoot" "INFO"
Write-Log "LogFile:  $script:LogFile" "INFO"

# Detect web root: prefer .\web if exists
...
if (-not (Test-Path -LiteralPath $srcRoot)) {
  Write-Log "Cannot find src folder: $srcRoot" "FAIL"
  exit 2
}

# Backup target feature folder (if exists)
...
if (Test-Path -LiteralPath $featureDir) {
  Write-Log "Backing up existing folder: $featureDir" "INFO"
  Backup-Dir -SrcPath $featureDir -BackupRoot $backupRoot
  Save-LastBackupDir -RepoRoot $repoRoot -BackupDir $backupRoot
  Write-Log "Backup saved: $backupRoot" "PASS"
}

New-Dir $featureDir

# Best-effort install xlsx
try {
  $pkgPath = Join-Path $webRoot "package.json"
  if (Test-Path -LiteralPath $pkgPath) {
    $pkgJson = Get-Content -LiteralPath $pkgPath -Raw -Encoding UTF8
    if ($pkgJson -notmatch '"xlsx"\s*:') {
      Write-Log "Installing dependency: xlsx" "INFO"
      Run-Npm -WorkingDir $webRoot -Args @("i","xlsx")
    } else {
      Write-Log "Dependency already exists: xlsx" "PASS"
    }
  } else {
    Write-Log "No package.json at webRoot -> skip npm install" "WARN"
  }
} catch {
  Write-Log ("npm install xlsx failed (continue): " + $_.Exception.Message) "WARN"
}
...
Write-Log "DONE. Created module at: $featureDir" "PASS"
Write-Log "NEXT: Add route -> /admin/data-upload => <AdminExcelUploadPage />" "INFO"
