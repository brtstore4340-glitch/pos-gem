# tools/patch-firebase-cd.ps1  (NO-EXPANSION SAFE VERSION)
[CmdletBinding()]
param(
...
"@
  ...
  # 3) Preview workflow (literal, then replace placeholders)
    ...
    steps:
      ...
      - name: Deploy to Firebase Hosting preview channel
        ...
        with:
          ...
          firebaseServiceAccount: ${{ secrets.FIREBASE_SERVICE_ACCOUNT___PROJECT_ID__ }}
          projectId: __PROJECT_ID__
'@

  $preview = $preview.Replace("__NODE_VERSION__", "$NodeVersion")
  $preview = $preview.Replace("__PROJECT_ID__", $ProjectId)
  $preview = $preview.Replace("FIREBASE_SERVICE_ACCOUNT___PROJECT_ID__", "FIREBASE_SERVICE_ACCOUNT_$ProjectId")
  WriteFileUtf8 $previewPath $preview

  # 4) Live deploy workflow (literal, then replace placeholders)
...
'@
  ...
  $live = $live.Replace("__NODE_VERSION__", "$NodeVersion")
  WriteFileUtf8 $livePath $live

  Step "DONE"
  Step "Required secrets:"
  Step "1) FIREBASE_SERVICE_ACCOUNT_$ProjectId  (service account JSON)  -> preview"
  Step "2) GCP_SA_KEY                           (service account JSON)  -> live deploy auth"
  Step "3) FIREBASE_PROJECT_ID                  ($ProjectId)            -> live deploy target"
