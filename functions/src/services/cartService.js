...
const calculateLine = (rawItem) => {
  ...
  const __thamMapped =
    typeof thamMapFields === "function" ? thamMapFields(rawItem || {}) : null;
  if (__thamMapped) {
    const __n = String(__thamMapped.name ?? "").trim();
    if ((!item.name || String(item.name).trim() === "") && __n) item.name = __n;

    const __u = Number.isFinite(+__thamMapped.unitPrice)
      ? +__thamMapped.unitPrice
      : 0;
    if ((!Number.isFinite(+item.unitPrice) || +item.unitPrice <= 0) && __u > 0)
      item.unitPrice = __u;

    const __dp = Number.isFinite(+__thamMapped.dealPrice)
      ? +__thamMapped.dealPrice
      : 0;
    if ((!Number.isFinite(+item.dealPrice) || +item.dealPrice <= 0) && __dp > 0)
      item.dealPrice = __dp;
    ...
    if (
      (__baseMethod === "" || __baseMethod === "0") &&
      __mappedMethod !== "" &&
      __mappedMethod !== "0"
    )
      item.method = __mappedMethod;
  }
  ...
  if (mapped) {
    ...
    if ((!item.name || String(item.name).trim() === "") && mappedName)
      item.name = mappedName;

    const mappedUnit = Number.isFinite(+mapped.unitPrice)
      ? +mapped.unitPrice
      : 0;
    if (
      ...
      item.unitPrice = mappedUnit;

    const mappedDealPrice = Number.isFinite(+mapped.dealPrice)
      ? +mapped.dealPrice
      : 0;
    if (
      (!Number.isFinite(+item.dealPrice) || +item.dealPrice <= 0) &&
      mappedDealPrice > 0
    )
      item.dealPrice = mappedDealPrice;

    const mappedDealQty = parseInt(String(mapped.dealQty ?? 0).trim(), 10);
    if (
      (parseInt(String(item.dealQty ?? 0).trim(), 10) || 0) <= 0 &&
      Number.isFinite(mappedDealQty) &&
      mappedDealQty > 0
    )
      item.dealQty = mappedDealQty;

    const baseMethod = String(item.method ?? "0").trim();
    const mappedMethod = String(mapped.method ?? 0).trim();
    if (
      (baseMethod === "" || baseMethod === "0") &&
      mappedMethod !== "" &&
      mappedMethod !== "0"
    )
      item.method = mappedMethod;
  }
  // END:   THAM:APPLY_FIELD_MAP_TO_LINE_V1
  // BEGIN: THAM:CANON_METHOD_V1
  const __thamMethodNum = parseInt(String(item.method ?? "0").trim(), 10);
  item.method = Number.isFinite(__thamMethodNum)
    ? String(__thamMethodNum)
    : "0";
  item.qty = Math.max(0, parseInt(String(item.qty ?? 0).trim(), 10) || 0);
  item.dealQty = Math.max(
    0,
    parseInt(String(item.dealQty ?? 0).trim(), 10) || 0,
  );
  ...
  exports.calculateCartSummary = function (...args) {
    ...
    try {
      if (res && res.summary) {
        ...
        if (
          typeof oldNet === "number" &&
          Number.isFinite(oldNet) &&
          res.summary.netBeforeVat === undefined
        ) {
