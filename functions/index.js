...
const BOOTSTRAP_ADMIN_DOC = db
  .collection("system_metadata")
  .doc("bootstrap_admin_state");
...
function hashPin(pin, salt) {
  ...
  const hash = crypto
    .pbkdf2Sync(cleanPin, pinSalt, PIN_ITERATIONS, PIN_KEYLEN, "sha256")
    .toString("base64");
...
function sanitizePermissions(input, role) {
  const allowedMenus = Array.isArray(input?.allowedMenus)
    ? input.allowedMenus
    : DEFAULT_ALLOWED_MENUS[role] || [];
  const clean = Array.from(
    new Set(allowedMenus.map((m) => String(m || "").trim()).filter(Boolean)),
  );
...
async function getIdDocByCode(idCode) {
  ...
  if (!cleanId)
    throw new functions.https.HttpsError("invalid-argument", "idCode required");
  const indexSnap = await ID_INDEX.doc(cleanId).get();
  if (!indexSnap.exists)
    throw new functions.https.HttpsError("not-found", "ID not found");
  const email = String(indexSnap.data()?.email || "")
    .trim()
    .toLowerCase();
  if (!email)
    throw new functions.https.HttpsError(
...
async function getActor(context, actorIdCode) {
  ...
  if (targetEmail !== email)
    throw new functions.https.HttpsError(
      "permission-denied",
      "Actor email mismatch",
    );
  const data = idSnap.data() || {};
  if (data.status !== "active")
    throw new functions.https.HttpsError("permission-denied", "Actor disabled");
  return {
    email,
...
function assertManageScope(actor, targetEmail, targetRole, desiredRole) {
  ...
  if (actor.role !== "SM-SGM")
    throw new functions.https.HttpsError("permission-denied", "Not allowed");
  if (actor.email !== targetEmail)
    throw new functions.https.HttpsError(
      "permission-denied",
      "Email scope violation",
    );
...
async function acquireLock(lockOwner, type, fileMeta) {
  await db.runTransaction(async (tx) => {
    ...
    if (lock.inProgress) {
      throw new functions.https.HttpsError(
        "failed-precondition",
        `Upload in progress by ${lock.by || "unknown"}`,
      );
    }
    ...
    if ((type === "master" || type === "maintenance") && !pricingReady) {
      throw new functions.https.HttpsError(
        "failed-precondition",
        "ItemMasterPrintOnDeph (pricing) not uploaded yet",
      );
    }
...
function mapMasterRow(row) {
  const itemCode = safeStr(
    row.ProductCode || row.Itemcode || row.ItemCode || row.ID,
  );
...
exports.bootstrapAdmin = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      console.error(
        "bootstrapAdmin admin lookup failed, falling back to marker doc:",
        err?.message || err,
      );
    ...
    await db.runTransaction(async (tx) => {
      ...
      if ((await tx.get(indexRef)).exists)
        throw new functions.https.HttpsError(
          "already-exists",
          "ID already exists",
        );
      ...
      tx.set(
        accountRef,
        { email, createdAt: nowTs(), updatedAt: nowTs() },
        { merge: true },
      );
      tx.set(idRef, {
        idCode,
...
exports.bootstrapAdminHttp = functions
  .region(REGION)
  .https.onRequest((req, res) => {
    cors(req, res, async () => {
      try {
        ...
        if (!idCode || !pin)
          return res.status(400).json({ error: "idCode and pin required" });

        // Require Firebase Auth token (should be injected by proxy/emulator middleware)
        try {
          requireAuth({ auth: req.auth || req.user });
        } catch (e) {
        ...
        const result = await callable.run({
          data: { idCode, pin },
          context: { auth: req.auth || req.user },
        });
...
exports.verifyIdPin = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (idData.status !== "active")
      throw new functions.https.HttpsError(
        "failed-precondition",
        "ID disabled",
      );

    const lockedUntil = idData.pinLockedUntil?.toMillis
      ? idData.pinLockedUntil.toMillis()
      : 0;
    ...
    const permissions =
      idData.permissions || sanitizePermissions(null, idData.role);
...
exports.createId = functions
  .region(REGION)
  .https.onCall(async (data, context) => {
    const actor = await getActor(context, data?.actorIdCode);
    if (!["admin", "SM-SGM"].includes(actor.role))
      throw new functions.https.HttpsError("permission-denied", "Not allowed");
    ...
    await db.runTransaction(async (tx) => {
      ...
      tx.set(
        accountRef,
        { email: targetEmail, createdAt: nowTs(), updatedAt: nowTs() },
        { merge: true },
      );
      tx.set(idRef, {
        idCode,
...
exports.adminResetAllPins = functions
  .region(REGION)
  .https.onCall(async (data, context) => {
    const actor = await getActor(context, data?.actorIdCode);
    if (actor.role !== "admin")
...
exports.updateId = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (data?.permissions)
      updates.permissions = sanitizePermissions(
        data?.permissions,
        desiredRole || targetData.role,
      );
...
exports.getAuditLogs = functions
  .region(REGION)
  .https.onCall(async (data, context) => {
    const actor = await getActor(context, data?.actorIdCode);
    if (!["admin", "SM-SGM"].includes(actor.role))
    ...
    if (actor.role === "SM-SGM" && !targetEmail)
      query = query.where("targetEmail", "==", actor.email);
...
exports.calculateOrder = functions
  .region(REGION)
  .https.onCall(async (data, context) => {
    requireAuth(context);
    ...
    if (!actorIdCode)
      throw new functions.https.HttpsError(
        "invalid-argument",
        "actorIdCode required",
      );
    const actor = await getActor(context, actorIdCode);
    assertMenuAccess(actor, "pos");
    const { items, billDiscountPercent, coupons, allowance, topup } =
      data || {};
...
exports.uploadChunk = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (rows.length === 0)
      return { ok: true, processed: 0, matched: 0, skipped: 0, invalid: 0 };
    ...
    if (!lock.inProgress || lock.by !== actor.idCode || lock.type !== type) {
      throw new functions.https.HttpsError(
        "failed-precondition",
        "No active lock for this upload",
      );
    }
    ...
    if (type === "pricing") {
      ...
      for (const r of rows) {
        ...
        ops.push({
          ref: db.collection("products").doc(itemCode),
          data: doc,
          merge: true,
        });
        ...
        if (barcode) {
          ops.push({
            ref: db.collection("barcode_index").doc(barcode),
            data: { itemCode, updatedAt: nowTs() },
            merge: true,
          });
        }
        processed++;
      ...
      for (const c of codeChunks) {
        ...
        snaps.forEach((snap, idx) => {
          if (snap.exists) existsSet.add(c[idx]);
        });
      }

      for (const m of mappedRows) {
        if (!existsSet.has(m.itemCode)) {
          skipped++;
          continue;
        }
        ops.push({
          ref: db.collection("products").doc(m.itemCode),
          data: m.upd,
          merge: true,
        });
        processed++;
...
exports.finalizeUpload = functions
  .region(REGION)
  .https.onCall(async (data, context) => {
    requireAuth(context);
    requireAppCheck(context);
