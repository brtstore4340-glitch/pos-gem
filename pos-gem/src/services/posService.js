  ...
  searchProducts: async (keyword) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Strategy 1 failed:", e.code);
      }
      ...
      if (results.length < 5) {
        try {
          ...
          console.warn("Strategy 2 failed:", e.code);
        }
      }
  ...
  uploadFileOptimized: async (file, type, onProgress) => {
    ...
    rowsToProcess.forEach((row, index) => {
      try {
        ...
        if (type === "MASTER") {
          const status = String(
            row["ProductStatus"] || row["Product Status"] || "",
          ).trim();
          ...
          updateData = {
            ...
            ProductDesc: String(
              row["ProductDesc"] || row["Description"] || "",
            ).trim(),
