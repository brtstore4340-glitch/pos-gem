        ...
        <div className="w-[350px] flex flex-col border-r border-slate-200 bg-white shrink-0">
          <div className="p-4 border-b border-slate-100 bg-slate-50/50">
            ...
            <div className="relative group">
              ...
              />
              <Search
                className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-boots-base transition-colors"
                size={18}
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto bg-slate-50 p-2 space-y-2">
            {loading ? (
              ...
              <div className="text-center py-20 text-slate-400 flex flex-col items-center">
                ...
                <span>
                  {keyword.length < 2 ? "พิมพ์เพื่อค้นหา" : "ไม่พบสินค้า"}
                </span>
              </div>
            ) : (
              results.map((item) => (
                ...
                >
                  {selectedItem?.sku === item.sku && (
                    <div className="absolute left-0 top-0 bottom-0 w-1 bg-boots-base"></div>
                  )}
                  <div className="pl-2">
                    <div
                      className={cn(
            ...
            <div className="flex-1 overflow-y-auto p-6 lg:p-8">
              <div className="max-w-5xl mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-8 opacity-5 text-boots-base pointer-events-none">
                    <Package size={200} />
                  </div>
                  <div className="relative z-10 flex flex-col md:flex-row gap-6 justify-between items-start border-b border-slate-100 pb-6 mb-6">
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <span className="bg-boots-base text-white text-[10px] px-2 py-0.5 rounded-full font-bold tracking-wide uppercase">
                          Master Product
                        </span>
                        <span
                          className={cn(
                            "text-[10px] px-2 py-0.5 rounded-full border font-bold",
                            selectedItem.ProductStatus?.startsWith("0")
                              ? "text-green-600 border-green-200 bg-green-50"
                              : "text-red-600 border-red-200 bg-red-50",
                          )}
                        >
                          {selectedItem.ProductStatus || "Unknown"}
                        </span>
                      </div>
                      <h1 className="text-2xl md:text-3xl font-bold text-slate-800 leading-tight mb-2">
                        {selectedItem.ProductDesc}
                      </h1>
                      <div className="flex flex-wrap gap-4 text-sm text-slate-500 font-mono mt-2">
                        <span className="flex items-center gap-1">
                          <Database size={14} /> ID: {selectedItem.sku}
                        </span>
                    ...
                    <div className="text-right bg-slate-50 p-4 rounded-xl border border-slate-100 min-w-[140px]">
                      ...
                      <div className="text-4xl font-bold text-boots-base tracking-tight">
                        ฿{selectedItem.SellPrice?.toLocaleString()}
                      </div>
                      <div className="text-[10px] text-slate-400 mt-1">
                        Vat ({selectedItem.VatRate}%)
                      </div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                    <DetailItem
                      ...
                      sub={selectedItem.DeptDesc}
                    />
                    <DetailItem
                      label="Group"
                      value={selectedItem.GroupCode}
                      sub={selectedItem.GroupDesc}
                    />
                    <DetailItem
                      label="Sub Group"
                      value={selectedItem.SubGroupCode}
                    />
                    <DetailItem
                      label="Supplier"
                      value={selectedItem.SupplierCode}
                      sub={selectedItem.SupplierName}
                    />
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                  <div className="px-6 py-4 border-b border-slate-100 bg-blue-50/30 flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-slate-800">
                      ข้อมูล Print On Dept (ครบทุกฟิลด์)
                    </h3>
                  </div>
                  <div className="p-6">
                    {/* Description Print */}
                    <div className="mb-4 pb-4 border-b border-slate-50">
                      <DetailItem
                        ...
                        value={selectedItem.description_print}
                        fullWidth
                      />
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4">
                      ...
                      <DetailItem
                        label="Brand"
                        value={selectedItem.brand_print}
                      />
                      <DetailItem
                        label="Merchandise"
                        value={selectedItem.merchandise}
                      />

                      <DetailItem
                        label="Reg Price"
                        value={selectedItem.regPrice_print}
                      />
                      <DetailItem
                        label="Unit Price"
                        value={selectedItem.unitPrice_print}
                      />
                      <DetailItem
                        label="Tax Code"
                        value={selectedItem.tax_print}
                      />
                      <DetailItem
                        label="Method"
                        value={selectedItem.method_print}
                      />

                      <div className="col-span-2 bg-yellow-50 p-3 rounded-lg border border-yellow-100 flex justify-between items-center">
                        <DetailItem
                          label="Deal Price"
                          value={selectedItem.dealPrice_print}
                          highlight
                        />
                        <DetailItem
                          label="Deal Qty"
                          value={selectedItem.dealQty_print}
                          highlight
                        />
                        <DetailItem
                          label="Limit"
                          value={selectedItem.limit_print}
                        />
                      </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                  <div className="px-6 py-4 border-b border-slate-100 bg-orange-50/30 flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-slate-800">
                      ข้อมูล Maintenance Event (ครบทุกฟิลด์)
                    </h3>
                  </div>
                  <div className="p-6">
                    {/* Description Maint */}
                    <div className="mb-4 pb-4 border-b border-slate-50">
                      <DetailItem
                        label="Description (Maint)"
                        value={selectedItem.description_maint}
                        fullWidth
                      />
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4">
                      <DetailItem
                        label="Event Type"
                        value={selectedItem.type_maint}
                      />
                      <DetailItem
                        label="Dept"
                        value={selectedItem.dept_maint}
                      />
                      <DetailItem
                        label="Class"
                        value={selectedItem.class_maint}
                      />
                      <DetailItem
                        label="MP Group"
                        value={selectedItem.mpGroup_maint}
                      />
                      <DetailItem
                        label="Reg Price"
                        value={selectedItem.regPrice_maint}
                      />
                      <DetailItem
                        label="Unit Price"
                        value={selectedItem.unitPrice_maint}
                      />
                      <DetailItem
                        label="Method"
                        value={selectedItem.method_maint}
                      />
                      <div className="hidden md:block"></div> {/* Spacer */}
                      <div className="col-span-2 bg-orange-50 p-3 rounded-lg border border-orange-100 flex justify-between items-center">
                        <DetailItem
                          label="Deal Price"
                          value={selectedItem.dealPrice_maint}
                          highlight
                        />
                        <DetailItem
                          label="Deal Qty"
                          value={selectedItem.dealQty_maint}
                          highlight
                        />
                        <DetailItem
                          label="Limit Qty"
                          value={selectedItem.limitQty_maint}
                        />
                      </div>
                    </div>
                  </div>
    ...
    <div className={cn("min-w-0", fullWidth && "w-full")}>
      ...
      <div
        className={cn(
          ...
          highlight ? "text-slate-800 font-bold text-lg" : "text-slate-700",
      ...
      {sub && (
        <div className="text-[10px] text-slate-400 truncate" title={sub}>
          {sub}
        </div>
      )}
    </div>
