    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
        ...
        <div className="bg-white w-full max-w-[380px] rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div
            className="p-6 overflow-y-auto font-mono text-sm bg-white"
            id="receipt-area"
          >
            {/* Logo & Header */}
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-slate-500 text-xs">
                ...
                <span>
                  {(order.summary.subtotal / 1.07).toLocaleString(undefined, {
              ...
              {order.adjustments?.coupons?.length > 0 && (
                <div className="flex justify-between text-slate-500 text-xs">
                  ...
                  <span className="text-red-600">
                    -
                    {order.adjustments.coupons
                      .reduce((a, c) => a + c.couponValue, 0)
                      .toLocaleString()}
                  </span>
                </div>
              )}
              {order.adjustments?.allowance > 0 && (
                <div className="flex justify-between text-slate-500 text-xs">
                  <span>Allowance</span>
                  <span className="text-red-600">
                    -{Number(order.adjustments.allowance).toLocaleString()}
                  </span>
                </div>
              )}
              {order.adjustments?.topup > 0 && (
                <div className="flex justify-between text-slate-500 text-xs">
                  ...
                  <span className="text-red-600">
                    -{Number(order.adjustments.topup).toLocaleString()}
                  </span>
                </div>
              )}
              {order.adjustments?.billDiscount?.amount > 0 && (
                <div className="flex justify-between text-slate-500 text-xs">
                  <span>
                    Bill Discount ({order.adjustments.billDiscount.percent}%)
                  </span>
                  <span className="text-red-600">
                    -
                    {Number(
                      order.adjustments.billDiscount.amount,
                    ).toLocaleString()}
                  </span>
                </div>
              )}
              ...
              <div className="flex justify-between text-slate-800 font-bold text-lg mt-2">
                ...
                <span>
                  ฿
                  {(
                    order.summary.netTotal || order.summary.subtotal
                  ).toLocaleString()}
                </span>
              </div>
            </div>
