...
export default function ManagementConsole() {
  ...
  const roleOptions = useMemo(
    () => (isAdmin ? ROLE_OPTIONS : ROLE_OPTIONS.filter((r) => r !== "admin")),
    [isAdmin],
  );

  if (!canAccess) {
    return (
      <AccessDenied message="You do not have access to management console." />
    );
  }
  ...
  const handleLoadAudit = async () => {
    try {
      const logs = await getAuditLogs({
        actorIdCode: session.idCode,
        email: isAdmin ? searchEmail || undefined : session.email,
      });
      setAuditLogs(logs);
    } catch (e) {
      setAuditLogs([]);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Manage user IDs, roles, and system permissions
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {searchResults.length > 0 && (
            <div className="mt-2 p-3 rounded-xl bg-slate-50 dark:bg-white/5 max-h-40 overflow-y-auto space-y-2">
              ...
              {searchResults.map((id, idx) => (
                <div
                  key={idx}
                  className="flex justify-between items-center text-sm p-2 bg-white dark:bg-black/20 rounded-lg border border-slate-100 dark:border-white/5"
                >
                  <div>
                  ...
                  <div className="px-2 py-1 rounded bg-slate-100 dark:bg-white/10 text-xs font-bold uppercase">
                    {id.role}
                  </div>
                </div>
              ))}
              ...
              <div className="flex flex-wrap gap-2">
                {MENU_OPTIONS.map((menu) => (
                  <label
                    key={menu}
                    className={cn(
                      "text-xs border rounded-lg px-2 py-1.5 cursor-pointer flex items-center gap-2 transition-all",
                  ...
                  >
                    ...
                    />
                    {newMenus.includes(menu) && <CheckCircle size={10} />}{" "}
                    {menu}
                  </label>
                ))}
              </div>
        ...
        <div className="glass-panel p-6 flex flex-col gap-4">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => handleResetPin(editIdCode)}
              className="btn-secondary flex-1 border-orange-200 text-orange-600 hover:bg-orange-50 dark:border-orange-500/30 dark:text-orange-400 dark:hover:bg-orange-500/10"
            >
        ...
        <div className="glass-panel p-6 flex flex-col gap-4 relative overflow-hidden">
          <div className="flex items-center justify-between mb-2 pb-2 border-b border-slate-100 dark:border-white/5">
            ...
            <button
              ...
              className="text-xs font-bold px-2 py-1 rounded bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 transition-colors flex items-center gap-1"
            >
                  ...
                  {auditLogs.map((log) => (
                    ...
                    >
                      ...
                      <td className="p-3 text-slate-500 dark:text-slate-400 font-mono">
                        {log.targetIdCode || log.targetEmail}
                      </td>
