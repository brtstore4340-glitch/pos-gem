...
export default function AdminSettingsPage({ onBack }) {
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        try {
          ...
          const count = await posService.uploadProductAllDept(
            data,
            (curr, total) => {
              updateStatus("master", {
                progress: Math.round((curr / total) * 100),
              });
            },
          );
          ...
          updateStatus("master", {
            loading: false,
            log: { type: "success", msg: "✅ Upload Complete: " + count },
          });
          loadStats();
        } catch (err) {
          updateStatus("master", {
      ...
      error: () =>
        updateStatus("master", {
          loading: false,
          log: { type: "error", msg: "Read CSV Failed" },
        }),
    });
    e.target.value = "";
  };
  ...
  const handlePrintUpload = async (e) => {
    ...
    try {
      const count = await posService.uploadItemMasterPrint(
        file,
        (curr, total) => {
      ...
      updateStatus("print", {
        loading: false,
        log: { type: "success", msg: "✅ Updated " + count + " items" },
      });
    } catch (err) {
      updateStatus("print", {
        loading: false,
  ...
  const handleMaintUpload = async (e) => {
    ...
    try {
      const count = await posService.uploadItemMaintenance(
        file,
        (curr, total) => {
  ...
  const handleClearDatabase = async () => {
    ...
    updateStatus("master", {
      loading: true,
      log: { type: "info", msg: "Deleting..." },
    });
    try {
      await posService.clearDatabase();
      updateStatus("master", {
        loading: false,
        log: { type: "success", msg: "Database Cleared" },
      });
      loadStats();
    } catch (e) {
      ...
      updateStatus("master", {
        loading: false,
        log: { type: "error", msg: e?.message || "Failed to clear database" },
      });
    }
  };
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-slate-500 dark:text-slate-400">
            Configure global POS features and manage data
          </p>
        </div>
        {onBack && (
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              <div className="p-6 rounded-2xl bg-white dark:bg-black/20 border border-slate-200 dark:border-white/10 shadow-sm flex flex-col gap-4">
                <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                  <RefreshCw size={20} className="text-blue-500" /> Data Sync
                  Activity
                </h2>
                  ...
                  {statsLoading ? (
                    ...
                    uploadEntries.map((entry) => {
                      ...
                      return (
                        <div
                          key={entry.key}
                          className="p-3 rounded-xl border border-slate-100 dark:border-white/10 bg-slate-50/50 dark:bg-white/5 flex flex-col gap-1"
                        >
                ...
                <div className="flex flex-wrap gap-2 mt-6 relative z-10">
                  {hasMaster && (
                    <button
                      ...
                      className="btn-secondary border-red-200 text-red-600 hover:bg-red-50 dark:border-red-500/30 dark:text-red-400 dark:hover:bg-red-500/10 text-sm py-2"
                    >
                  ...
                  <a
                    ...
                    className="btn-secondary text-sm py-2 ml-auto"
                  >
        ...
        {activeSubTab === "features" && (
          <div className="max-w-3xl animate-fade-in-up">
            <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2">
              <Server size={22} className="text-blue-500" /> Feature
              Configuration
            </h2>
            ...
            <div className="grid gap-4">
              {[
                {
                  key: "multiStoreMode",
                  label: "Multi-Store Mode",
                  desc: "Enable syncing data across multiple branch locations",
                },
                {
                  key: "advancedPromotions",
                  label: "Advanced Promotions",
                  desc: "Allow complex discount rules and buy-1-get-1 logic",
                },
                {
                  key: "couponScanning",
                  label: "Coupon Scanning",
                  desc: "Enable barcode scanner support for physical coupons",
                },
                {
                  key: "voidManagerOnly",
                  label: "Manager Void Approval",
                  desc: "Require manager PIN to void items (Security)",
                },
                {
                  key: "autoReport",
                  label: "Auto-Send Reports",
                  desc: "Automatically email daily sales reports at closing",
                },
                {
                  key: "lowStockAlerts",
                  label: "Low Stock Alerts",
                  desc: "Notify cashier when inventory drops below threshold",
                },
                {
                  key: "thaiLanguagePriority",
                  label: "Thai Language Priority",
                  desc: "Default to Thai language for product names and receipts",
                },
              ].map(({ key, label, desc }) => (
                <div
                  key={key}
                  className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-xl border border-slate-100 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group"
                >
                  <div className="pr-0 sm:pr-4 min-w-0 space-y-1">
                    <div className="font-bold text-slate-800 dark:text-white leading-snug break-words">
                      {label}
                    </div>
                    <div className="text-sm text-slate-500 dark:text-slate-400 leading-snug break-words">
                      {desc}
                    </div>
                  </div>
                  <button
                    ...
                    className={cn(
                      "relative inline-flex h-7 w-12 items-center rounded-full transition-colors self-start sm:self-center",
                      features[key]
                        ? "bg-blue-600"
...
function UploadCard({
  ...
  return (
    <div
      className={cn(
        "bg-white dark:bg-black/20 rounded-2xl shadow-sm border overflow-hidden relative transition-all h-full flex flex-col",
        active
          ? "border-slate-200 dark:border-white/10 hover:shadow-md hover:border-blue-200 dark:hover:border-blue-500/30"
          : "border-slate-100 dark:border-white/5 opacity-60 grayscale",
      )}
    >
      {!active && (
      ...
      <div className="p-6 border-b border-slate-50 dark:border-white/5 flex justify-between items-start bg-slate-50/50 dark:bg-white/5">
        <div>
          ...
          {required && (
            <span className="text-[10px] bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400 px-2 py-0.5 rounded-full mt-1 inline-block font-bold">
              REQUIRED FIRST
            </span>
          )}
        </div>
        <div
          className={cn(
            ...
            active
              ? "bg-white dark:bg-white/10 text-boots-base dark:text-blue-400 border border-blue-100 dark:border-white/10"
      ...
      <div className="p-6 flex-1 flex flex-col min-w-0">
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-4 flex-1 leading-relaxed break-words">
          {desc}
        </p>
        <div className="text-xs text-slate-500 dark:text-slate-400 mb-4 flex items-center gap-2 flex-wrap">
          ...
          <span className="font-medium">Last upload:</span>
          <span className="font-mono">
            {formatUploadDate(meta?.lastUploadAt)}
          </span>
          {meta?.count ? (
            <span className="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-white/10 border border-slate-200 dark:border-white/10 font-semibold text-[11px] text-slate-700 dark:text-slate-200">
              {meta.count.toLocaleString()} rows
            </span>
          ) : null}
        </div>
        ...
        {!status.loading ? (
          <div className="relative group cursor-pointer">
            ...
            <div className="border-2 border-dashed border-slate-300 dark:border-white/10 rounded-xl p-6 text-center group-hover:bg-blue-50 dark:group-hover:bg-blue-500/10 group-hover:border-blue-300 dark:group-hover:border-blue-500/30 transition-all bg-slate-50 dark:bg-white/5">
              <Upload
                size={24}
                className="mx-auto text-slate-400 mb-2 group-hover:text-blue-500"
              />
          ...
          <div className="py-8 text-center bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-100 dark:border-white/10">
            <RefreshCw
              size={24}
              className="mx-auto animate-spin text-boots-base dark:text-blue-400 mb-3"
            />
        ...
        {status.log && (
          <div
            className={cn(
              "mt-4 p-3 rounded-lg text-xs flex items-center gap-2 animate-in slide-in-from-bottom-2",
