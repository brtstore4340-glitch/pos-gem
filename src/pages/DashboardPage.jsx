    ...
    <div className="p-6 md:p-8 w-full max-w-7xl mx-auto space-y-8 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {stats.map((stat, idx) => (
          <div
            key={idx}
            className="glass-panel p-6 relative overflow-hidden group hover:scale-[1.01] transition-transform"
          >
            <div className={`absolute top-0 right-0 p-4 opacity-50`}>
              <stat.icon
                size={64}
                className={`${stat.color} opacity-10 group-hover:scale-110 transition-transform`}
              />
            </div>
