...
import { UploadCloud, File, X, HardDrive, CheckCircle, AlertCircle, Loader2 } from "lucide-react";
...
export default function FileUploadPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setStatus({ type: 'error', message: error.message || "Upload failed. Please check console." });
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="h-full p-6 md:p-8 animate-fade-in-up flex flex-col gap-6 max-w-4xl mx-auto">
       ...
       <div className="glass-panel p-8 rounded-3xl min-h-[400px] flex flex-col items-center justify-center border-dashed border-2 border-slate-300 dark:border-white/10 bg-slate-50/50 dark:bg-white/5 relative transition-colors"
       ...
       >
          ...
          <div className="flex flex-col items-center text-center gap-4 pointer-events-none">
             <div className={`w-24 h-24 rounded-full flex items-center justify-center mb-2 transition-all ${uploading ? 'bg-blue-100 text-blue-600 animate-pulse' : 'bg-blue-50 dark:bg-blue-900/20 text-blue-500 animate-bounce-slow'}`}>
                {uploading ? <Loader2 size={48} className="animate-spin" /> : <UploadCloud size={48} />}
             </div>
          ...
          {status && !uploading && (
              <div className={`absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-3 rounded-xl flex items-center gap-3 shadow-lg ${status.type === 'success' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200'}`}>
                  {status.type === 'success' ? <CheckCircle size={20} /> : <AlertCircle size={20} />}
                  <span className="font-medium text-sm">{status.message}</span>
                  <button onClick={(e) => { e.stopPropagation(); setStatus(null); }} className="p-1 hover:bg-black/5 rounded-full"><X size={14}/></button>
              </div>
          )}
       </div>
    </div>
