...
export async function fetchData(path, options = {}) {
  try {
    ...
    const qy = cursor ? query(collection(db, path), startAfter(cursor), limit(pageSize)) : base;
    ...
    return { success: false, data: [], nextCursor: null, error: error?.message || 'Unknown error' };
  }
}
