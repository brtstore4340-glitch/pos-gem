...
function AppShell() {
  ...
  const allowedMenus = useMemo(() => {
    ...
    const base =
      Array.isArray(explicit) && explicit.length
        ? explicit
        : DEFAULT_ALLOWED_MENUS[session.role] || [];
  ...
  const visibleTabs = useMemo(
    () => tabs.filter((tab) => allowedSet.has(tab.id)),
    [tabs, allowedSet],
  );
  ...
  return (
    <div className="min-h-dvh h-dvh bg-[#f8fafc] dark:bg-[#0f1014] text-slate-800 dark:text-slate-200 flex flex-col overflow-hidden font-sans transition-colors duration-300 relative">
      <ErrorBoundary>
            ...
            <div className="flex items-center gap-4">
              <div className="relative group">
                ...
                <img
                  src={logoSrc}
                  alt="Boots Logo"
                  className="h-9 w-auto object-contain relative transition-transform hover:scale-105"
                />
              </div>
              <div className="hidden md:flex flex-col">
                <span className="text-xs font-bold text-slate-700 dark:text-slate-200 leading-none tracking-tight">
                  POS TERMINAL
                </span>
                ...
                {visibleTabs.map((tab) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <span
                        className={`text-[10px] font-semibold leading-none ${isActive ? "text-blue-700 dark:text-blue-300" : "text-slate-500 dark:text-slate-400"}`}
                      >
                        {tab.label}
                      </span>
                    </button>
        ...
        {/* Main Content Area */}
        <main
          className={`flex-1 relative z-0 transition-all ${activeTab === "pos" ? "overflow-hidden" : "overflow-auto px-4 pb-4"}`}
        >
          <AnimatePresence mode="wait">
            ...
            >
              <Suspense fallback={<TabSkeleton />}>
                ...
                <div className="h-full w-full">{renderActiveTab()}</div>
              </Suspense>
            </motion.div>
          </AnimatePresence>
        </main>
