        ...
        <label className={`block w-full`}>
          ...
          >
            <div className="text-sm text-zinc-800 dark:text-zinc-200">
              {file
                ? `Selected: ${file.name}`
                : "Click to choose Excel file (.xlsx)"}
            </div>
...
export default function AdminExcelUploadPage() {
  const [statusMeta, setStatusMeta] = useState<Record<string, unknown> | null>(
    null,
  );
  ...
  async function handleUploadAll() {
    ...
    try {
      ...
      if (!masterKeyField)
        throw new Error(
          "Cannot detect master key column (ItemCode/SKU/Barcode).",
        );
      ...
      setProgress({
        phase: "filtering",
        percent: 8,
        message: "Filtering status=0 in master...",
      });

      const { filtered: masterFiltered, keySet } = filterMasterStatus0(
        masterRows,
        masterKeyField,
        statusField,
      );
      setProgress({
        phase: "uploading",
        percent: 10,
        message: `Uploading master (${masterFiltered.length} rows)...`,
      });
      ...
      if (itemMasterFile) {
        setProgress({
          phase: "reading",
          percent: 1,
          message: "Reading itemmaster file...",
        });
        ...
        setProgress({
          phase: "filtering",
          percent: 6,
          message: "Filtering itemmaster by master keys...",
        });
        ...
        setProgress({
          phase: "uploading",
          percent: 10,
          message: `Uploading itemmaster (${filtered.length} rows)...`,
        });
        await uploadRowsChunked({
          kind: "itemmaster",
          rows: filtered,
      ...
      if (itemEventFile) {
        setProgress({
          phase: "reading",
          percent: 1,
          message: "Reading itemevent file...",
        });
        ...
        setProgress({
          phase: "filtering",
          percent: 6,
          message: "Filtering itemevent by master keys...",
        });
        const filtered = filterByKeySet(rows, keyField, keySet);
        ...
        setProgress({
          phase: "uploading",
          percent: 10,
          message: `Uploading itemevent (${filtered.length} rows)...`,
        });
        await uploadRowsChunked({
          kind: "itemevent",
          rows: filtered,
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <div className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
                Master = Itemmasterprintondept (status=0 only) •
                Itemmaster/Itemevent upload only items inside Master
              </div>
            </div>
            ...
            <div className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
              <div className="rounded-xl bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 p-3">
                ...
                <div className="font-medium">
                  {fmtIso(statusMeta?.master?.lastUploadedAt)}
                </div>
              </div>
              <div className="rounded-xl bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 p-3">
                <div className="text-xs text-zinc-500 dark:text-zinc-400">
                ...
                <div className="font-medium">
                  {fmtIso(statusMeta?.itemmaster?.lastUploadedAt)}
                </div>
              </div>
              <div className="rounded-xl bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 p-3">
                <div className="text-xs text-zinc-500 dark:text-zinc-400">
                ...
                <div className="font-medium">
                  {fmtIso(statusMeta?.itemevent?.lastUploadedAt)}
                </div>
              </div>
            </div>

            <div className="mt-3 text-xs text-zinc-500 dark:text-zinc-400">
              Collections: masterPrintOnDept • itemMaster • itemEvent • Meta:
              adminMeta/uploadStatus
            </div>
          </div>
        </div>
      </div>
