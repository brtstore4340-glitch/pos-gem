...
export default function AdminSettings({ onBack }) {
  ...
  const handlePrintUpload = async (e) => {
    ...
    try {
      ...
      updateStatus("print", {
        loading: false,
        log: { type: "success", msg: "✅ Updated " + count + " matched items" },
      });
    } catch (err) {
      updateStatus("print", {
        loading: false,
  ...
  const handleClearDatabase = async () => {
    ...
    updateStatus("master", {
      loading: false,
      log: { type: "success", msg: "Deleted All" },
    });
    loadStats();
  };
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-boots-base">
                Data Management Center
              </h1>
              <p className="text-sm text-slate-500">
                จัดการข้อมูลสินค้าและอัพเดทฐานข้อมูล
              </p>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            {isSearching && (
              <div className="absolute right-4 top-1/2 -translate-y-1/2">
                <RefreshCw className="animate-spin text-boots-base" size={20} />
              </div>
            )}
          </div>
...
function UploadCard({
  ...
  return (
    <div
      className={cn(
        "bg-white rounded-2xl shadow-sm border overflow-hidden relative transition-all h-full flex flex-col",
        active
    ...
    >
      ...
      <div className="p-6 border-b border-slate-50 flex justify-between items-start bg-slate-50/50">
        <div>
          ...
          {required && (
            <span className="text-[10px] bg-red-100 text-red-600 px-2 py-0.5 rounded-full mt-1 inline-block font-bold">
              REQUIRED FIRST
            </span>
          )}
        </div>
        <div
          className={cn(
            ...
            active
              ? "bg-white text-boots-base border border-blue-100"
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        {!status.loading ? (
          ...
          <div className="py-8 text-center bg-slate-50 rounded-xl border border-slate-100">
            <RefreshCw
              size={24}
              className="mx-auto animate-spin text-boots-base mb-3"
            />
