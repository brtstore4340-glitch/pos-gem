...
export default function PosUploadModal({ open, onClose, isDarkMode = false }) {
  ...
  const statusBadge = useMemo(() => {
    ...
    if (result)
      return { icon: <CheckCircle size={16} />, text: "Upload completed" };
  ...
  const onPick = async (type, file) => {
    ...
    try {
      const summary = await runUploadFlow({
        actorIdCode,
        type,
        file,
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      ...
      >
        ...
        <div
          className={cn("px-6 py-4", isDarkMode ? "bg-slate-950" : "bg-white")}
        >
          <div
            className={cn(
          ...
          {result && (
            ...
            >
              ...
              <div className="text-xs">
                processed: {result.summary.processed} | matched:{" "}
                {result.summary.matched} | skipped: {result.summary.skipped} |
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            <input
              ...
              accept=".csv"
              disabled={busy}
              onChange={(e) => onPick("master", e.target.files?.[0])}
          ...
          <Card
            ...
            subtitle={
              canSecondary
                ? "merge เฉพาะ fields ที่กำหนด"
                : "ต้องอัปโหลด Master ก่อน"
            }
          ...
          >
            <input
              ...
              accept=".xls,.xlsx"
              disabled={busy || !canSecondary}
              onChange={(e) => onPick("pricing", e.target.files?.[0])}
              className="w-full text-xs"
            />
          </Card>
          ...
          >
            <input
              ...
              accept=".xls,.xlsx"
              disabled={busy || !canSecondary}
              onChange={(e) => onPick("maintenance", e.target.files?.[0])}
              className="w-full text-xs"
            />
          </Card>
        </div>
        ...
        <div className="px-6 pb-6 flex justify-end gap-3">
          {busy && (
            <button
              onClick={abortNow}
              className="px-4 py-2 rounded-xl border border-red-500/40 bg-red-500/10 text-red-200 font-bold"
            >
              Abort
            </button>
          )}
